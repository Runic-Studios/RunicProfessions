...
public class WoodworkingMenuTutorial extends Workstation {

    public WoodworkingMenuTutorial(Player player) {
        super(WorkstationLoader.getMaxPages().get(WorkstationType.WOODWORKING_TABLE_TUTORIAL));
    ...
    private ItemGUI menu(Player player) {
        ...
        menu.setOption(4, new ItemStack(Material.IRON_AXE), "&eTutorial Woodworking Table",
                "&6&lClick &7an item to start crafting!", 1, false);

        super.setupItems(player, menu, WorkstationType.WOODWORKING_TABLE_TUTORIAL);
        ...
        menu.setHandler(event -> {
            ...
            if (event.getSlot() == 0) {
                ...
                if (meta == null) return;
                CraftedResource craftedResource = super.determineCraftedResource(WorkstationType.WOODWORKING_TABLE_TUTORIAL, event.getSlot());
                event.setWillClose(true);
                event.setWillDestroy(true);
                // todo: update
                startCrafting
